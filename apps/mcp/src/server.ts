#!/usr/bin/env node
/**
 * StockMaster draft-only MCP server entry point.
 *
 ...
 * StockMaster API per session, so the connection can only do what that key allows — i.e.
 * create/edit drafts, never publish or delete. Only STOCKMASTER_API_URL is server config.
 */
...
async function main(): Promise<void> {
  const apiUrl = process.env.STOCKMASTER_API_URL ?? '';
  if (!apiUrl) {
    console.error('[stockmaster-mcp] STOCKMASTER_API_URL is not set; API calls will fail.');
  }
...
main().catch((err) => {
  console.error('[stockmaster-mcp] fatal:', err instanceof Error ? err.stack ?? err.message : err);
  process.exit(1);
});
