...
### 4. Ship validate_content IN v1 via a React-free `@stockmaster/editor-schema` package — REVERSED 2026-06-02
...
**Verify:** From repo root run `pnpm --filter @stockmaster/api exec prisma validate` (schema parses) and `pnpm --filter @stockmaster/api exec prisma format` (no diff after). Confirm ApiKey appears and User.apiKeys back-relation resolves.
...
**Verify:** Inspect migration.sql: only CREATE TABLE/INDEX + FK, no destructive statements. `pnpm --filter @stockmaster/api exec prisma migrate status` shows the new migration applied and DB in sync. `pnpm --filter @stockmaster/api exec prisma generate` regenerates the client so prisma.apiKey delegate exists.
...
**Verify:** `pnpm --filter @stockmaster/api run typecheck` passes. In a scratch check, assigning req.scopes/req.authType in a guard compiles without `as any`. Audit existing @CurrentUser consumers (auth.controller.ts:67 me(), works.controller.ts:23 create()) read only user.id — confirmed safe with email optional.
...
**Verify:** `pnpm --filter @stockmaster/api run typecheck` passes. Decorators export the metadata keys ScopeGuard (step 7) imports. No runtime behavior yet.
...
**Change:** In toPage (serializers.ts:66) change the null-content fallback from `(p.content ?? { type: 'doc' })` to `(p.content ?? blankDoc())` importing blankDoc from '@stockmaster/shared'. blankDoc() returns { type:'doc', content:[{type:'paragraph'}] }, a valid editable template, so the MCP get_page echo is always a usable read-modify-write base.

**Verify:** `pnpm --filter @stockmaster/api run typecheck` passes. A page row with null content serialized via get_page returns a doc with a paragraph, not a bare {type:'doc'}. (Edge case in practice — all API-created pages already get blankDoc — but removes the malformed-template risk.)
...
**Verify:** `pnpm --filter @stockmaster/web run typecheck` and a dev run: log in, open API Keys, create a key (raw shown once, copyable, gone after close), list shows prefix/scopes, revoke marks revokedAt and the key stops working. Grep confirms rawKey is never passed to any persistence call.
...
**Verify:** `pnpm --filter @stockmaster/api test` runs green. The wildcard-regression and schema-drift tests are present and passing (these are the load-bearing backward-compat guarantees the design's test list at lines 142-148 omits).
...
### Step 16 — Create React-free `@stockmaster/editor-schema` package (canonical Tiptap schema)
...
- **✅ `validate_content` — BUILT IN v1.** Via the React-free `@stockmaster/editor-schema` package
